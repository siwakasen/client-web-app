import { fetchTravelPackages, fetchTravelPackagesDetail } from "@/_services/travel-packages";
...
export default async function TravelPackageDetailPage({params}: {params: Promise<{id: string}>}) {
    // Fetch data on the server
    const {id} = await params;
    const {data} = await fetchTravelPackagesDetail({id: Number(id)})
    const relatedPackages = await fetchTravelPackages({limit: 10, page: 1, search: ""})
    const filteredRelatedPackages = relatedPackages.data.filter((pkg) => pkg.id !== Number(id)).slice(0, 4)
    ...
    return (
      <div className="min-h-screen ">
        ...
        <div className="container mx-auto px-2 sm:px-8 md:px-8 lg:px-32 py-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="font-semibold">{formatPrice(data.package_price)}</p>
                  </CardContent>
                </Card>
              ...
              <Card className="sticky top-24">
                <CardHeader>
                  <div className="text-right">
                    ...
                    <div className="text-5xl font-bold text-primary">{formatPrice(data.package_price)}</div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground text-center pt-2">Price based on per person</p>
                  <Separator />
          ...
          {/* Related Packages */}
          <div className="mt-16">
            <h2 className="text-2xl font-bold mb-8">Explore More Package Tours</h2>
                ...
                >
                  <Card className="flex flex-col h-full rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow cursor-pointer bg-white p-0">
                    ...
                    <CardContent className="flex flex-col flex-1 p-5 bg-gradient-to-br from-white to-green-50">
                      <h3 className="font-semibold text-lg mb-2 line-clamp-2 text-gray-800 hover:text-green-700 transition-colors">{pkg.package_name}</h3>
