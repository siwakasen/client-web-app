...
export const TourPackageCard: React.FC<TourPackageCardProps> = ({ travelPackage }) => {
  return (
    <Link href={`/travel-packages/${travelPackage.id}`} className="hover:scale-103 transition-all duration-300 ">
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden  h-full flex flex-col">
        ...
        <div className="p-4 flex flex-col flex-grow">
          <div className="mb-4">
            <h3 className="text-lg font-semibold text-gray-800 text-md">{travelPackage.package_name}</h3>
            <p className="mt-2 text-sm text-gray-600">{travelPackage.description}</p>
          </div>
          ...
          <div className="mt-auto p-4 flex items-center justify-between bg-gray-100 border-gray-200 border-1  rounded-lg">
            <span className="text-gray-700 font-medium">$ {travelPackage.package_price}/person</span>
            <Button  className="bg-slate-700 hover:bg-slate-900 text-white font-bold py-2 px-4 rounded cursor-pointer">Book Now</Button>
          </div>
        </div>
      </div>
    </Link>
