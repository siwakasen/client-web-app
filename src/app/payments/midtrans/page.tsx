...
export default async function PaymentCompletePage({
  ...
  if ('errors' in response && 'status' in response) {
    ...
    if(payment.status === "SUCCESS" || transaction_status === "settlement" || transaction_status === "capture"){
      isSuccess = true;
      message = "Your payment has been successfull";
    } else if(payment.status === "PENDING"){
      isPending = true;
      message = "Your payment has not completed yet. Please click the button below to continue.";
    } else {
      isSuccess = false;
