...
export default async function PopularPackage() {
  ...
  const packages = data.map((pkg) => ({
    ...
    image: pkg.images && pkg.images.length > 0 ? convertTravelImageUrl(pkg.images[0]) : "https://images.unsplash.com/photo-1506748686214-e9df14d4d9d0?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
        ...
        <div className="flex flex-col md:flex-row w-full justify-between gap-8">
          {packages.length > 0 ? (
            packages.map((pkg, index) => (
              ...
              >
                <CardContent className={`p-0 w-full h-80 md:h-96 ${index === 0 ? 'md:min-w-96' : 'md:min-w-2xl'}`}>
