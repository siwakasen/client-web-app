...
export async function useCancelBooking(booking_id: number, reason: string) : Promise<CancelBookingResponse | { status: number; errors?: any }> {
    try {
...
export async function useRescheduleBooking(booking_id: number, new_start_date: string, new_end_date: string) : Promise<RescheduleBookingResponse | { status: number; errors?: any }> {
    try {
        const headers = await getHeaders();
        const token = await getToken();
