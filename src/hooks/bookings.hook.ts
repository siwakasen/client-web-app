...
export async function useGetBookingById(order_id: string): Promise<
  | BookingResponseById
  | {
      status: number;
      errors?: any;
...
> {
  try {
    const headers = await getHeaders();
    const token = await getToken();
    const response: BookingResponseById = await getBookingById(
      order_id,
      headers,
      token!
    );
...
export async function useGetBookingHistory(
  page: number,
  limit: number,
  status?: string
): Promise<
...
> {
  try {
    const headers = await getHeaders();
    const token = await getToken();
    const response: BookingHistoryResponse = await getBookingHistory(
      headers,
      token!,
      page,
      limit,
